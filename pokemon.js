/* Funções construtoras criam objetos "genéricos" para reaproveitamento
futuro do código, conforme exemplo abaixo.

JSON é um formato transacional usado para comunicação universal entre linguagens 
de programação e entre API's. 

Usamos JSON.parse para converter uma string JSON em objeto JS
e JSON.stringify para converter um objeto JS em uma string JSON. */
...
function proprioFilter(array, callback) {
  const filtrado = []
...
const pokemonsFortes = (proprioFilter(instanciasPokemons, valor => {
  return valor
}))
